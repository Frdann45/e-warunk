...
/**
 * =======
 * Warung Tiga Saudara - Kesehatan view
 * Author ID: 11240044
 * =======
 */
...
try {
    if ($searchQuery !== '') {
        $stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'Kesehatan' AND name LIKE ? ORDER BY id ASC LIMIT 12");
        $stmt->execute(['%' . $searchQuery . '%']);
    } else {
        $stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'Kesehatan' ORDER BY id ASC LIMIT 12");
        $stmt->execute();
    }
    $kesehatanProducts = $stmt->fetchAll();
} catch (PDOException $e) {
    $kesehatanProducts = [];
    error_log('Kesehatan fetch error: ' . $e->getMessage());
}
?>

<div class="catalog-header fade-in">
    <div>
        <h1 class="catalog-header__title">Katalog Kesehatan</h1>
        <p class="catalog-header__desc">Obat-obatan umum, vitamin, suplemen, dan perlengkapan kesehatan keluarga Anda.</p>
    </div>
    <div class="catalog-header__actions">
        ...
        <div class="search-box-wrapper">
            ...
            <form action="index.php" method="GET" style="margin:0; display:flex;">
                <input type="hidden" name="page" value="kesehatan">
                <input type="text" name="search" class="input-search" placeholder="Cari obat/alat kesehatan..." value="<?= htmlspecialchars($searchQuery) ?>">
            </form>
        </div>
    ...
    <div class="products__grid">
        <?php if (!empty($kesehatanProducts)): ?>
            <?php foreach ($kesehatanProducts as $product): ?>
                    ...
                    <div class="product-card__info">
                        ...
                        <div class="product-card__footer">
                            ...
                            <form action="cart_action.php" method="POST" style="margin:0;">
                                ...
                                <input type="hidden" name="redirect_page" value="kesehatan<?= $searchQuery !== '' ? '&search=' . urlencode($searchQuery) : '' ?>">
        ...
        <?php else: ?>
            <div style="grid-column: 1 / -1; text-align: center; padding: 48px 20px; color: var(--color-text-light);">
                <p>Tidak ada produk kategori Kesehatan.</p>
            </div>
        <?php endif; ?>
    </div>
