...
/**
 * =======
 * Warung Tiga Saudara - Perawatan & Kecantikan view
 * Author ID: 11240044
 * =======
 */
...
try {
    if ($searchQuery !== '') {
        $stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'Perawatan & Kecantikan' AND name LIKE ? ORDER BY id ASC LIMIT 10");
        $stmt->execute(['%' . $searchQuery . '%']);
    } else {
        $stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'Perawatan & Kecantikan' ORDER BY id ASC LIMIT 10");
        $stmt->execute();
    }
    $perawatanProducts = $stmt->fetchAll();
} catch (PDOException $e) {
    $perawatanProducts = [];
    error_log('Perawatan fetch error: ' . $e->getMessage());
}
?>

<div class="catalog-header fade-in">
    <div>
        <h1 class="catalog-header__title">Katalog Perawatan & Kecantikan</h1>
        <p class="catalog-header__desc">Produk perawatan tubuh, wajah, rambut, dan kecantikan pilihan terbaik untuk Anda.</p>
    </div>
    <div class="catalog-header__actions">
        ...
        <div class="search-box-wrapper">
            ...
            <form action="index.php" method="GET" style="margin:0; display:flex;">
                ...
                <input type="text" name="search" class="input-search" placeholder="Cari produk perawatan..." value="<?= htmlspecialchars($searchQuery) ?>">
            </form>
        </div>
    ...
    <div class="products__grid">
        <?php if (!empty($perawatanProducts)): ?>
            <?php foreach ($perawatanProducts as $product): ?>
                    ...
                    <div class="product-card__info">
                        ...
                        <div class="product-card__footer">
                            ...
                            <form action="cart_action.php" method="POST" style="margin:0;">
                                ...
                                <input type="hidden" name="redirect_page" value="perawatan<?= $searchQuery !== '' ? '&search=' . urlencode($searchQuery) : '' ?>">
        ...
        <?php else: ?>
            <div style="grid-column: 1 / -1; text-align: center; padding: 48px 20px; color: var(--color-text-light);">
                <p>Tidak ada produk kategori Perawatan & Kecantikan.</p>
            </div>
        <?php endif; ?>
    </div>
