...
/**
 * ============================================================
 * E-WARUNG (Warung Tiga Saudara) - Proses Pengiriman (Admin View)
 * ============================================================
 * Author ID   : 11240044
 * Created     : 2026-06-25
 * Description : View for monitoring orders that are currently
 *               shipped / in transit. Allows completing (Selesai)
 *               or cancelling (Dibatalkan) orders.
 * ============================================================
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($orderId > 0 && in_array($newStatus, ['Selesai', 'Dibatalkan'])) {
        try {
            ...
            if ($orderCode) {
                ...
                $_SESSION['cart_message'] = "Pesanan #{$orderCode} telah berhasil diselesaikan.";
            }
...
// ── Fetch orders with status = 'Dikirim' ──────────────────────
try {
    $stmt = $pdo->prepare("
        SELECT o.*, 
    ...
    error_log('Error fetching shipped orders: ' . $e->getMessage());
    $orders = [];
}
?>

<div class="history-header fade-in">
    <h1 class="history-header__title">Proses Pengiriman</h1>
    <p class="history-header__desc">Kelola pesanan yang saat ini dalam perjalanan / pengiriman kurir ke pelanggan.</p>
</div>
            ...
            <?php if (!empty($orders)): ?>
                <?php foreach ($orders as $order): ?>
                    ...
                    <tr>
                        ...
                        <td style="vertical-align: top; padding-top: 16px;">
                            <span class="status-badge status-badge--dikirim">
                        ...
                        <td style="vertical-align: top; padding-top: 12px; text-align: right; padding-right: 20px; white-space: nowrap;">
                            <div style="display: flex; gap: 8px; justify-content: flex-end;">
                                <form action="" method="POST" style="margin: 0;">
                                    ...
                                    <button type="submit" class="btn-admin btn-admin--primary" title="Tandai sebagai selesai / terkirim">
                                ...
                                <form action="" method="POST" style="margin: 0;" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan pengiriman #<?= htmlspecialchars($order['order_code']) ?>?');">
                                    <input type="hidden" name="update_status" value="1">
                                    <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="6" style="text-align: center; padding: 48px; color: var(--color-text-light);">
                        Tidak ada pesanan yang sedang dalam proses pengiriman saat ini.
                    </td>
                </tr>
            <?php endif; ?>
        </tbody>
